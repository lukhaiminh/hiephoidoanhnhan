...
class XenForo_ImageProxyStream
{
	...
	/**
	 * Max time in seconds to allow reading for
	 *
	 * @var int
	 */
	protected static $_maxReadTime = 10;
	...
	public function stream_write($data)
	{
		...
		if (!empty(self::$_metadata[$this->_path]['start']))
		{
			$spent = microtime(true) - self::$_metadata[$this->_path]['start'];
			if ($spent > self::$_maxReadTime)
			{
				self::$_metadata[$this->_path]['error'] = 'too_slow';
				return false;
			}
		}
