 ...
 * */
if (!class_exists('FAQWDLangClass')) {

    class FAQWDLangClass {
        ...
        public function __construct($post_type, $PluginName) {
            ...
            $this->SL_langDirPath = $upload_dir['basedir'] . '/Languages_WD/' . $this->SL_langPluginName . '/';
            $this->SL_langPluginDirPath = FAQ_DIR .'/languages/';
            $this->checkDirs();
        ...
        public function displayLaguageOptions() {
            if ($_POST && isset($_POST['task']) && $_POST['task'] == 'lang-delete') {
                check_admin_referer('delete_translation', 'delete_translation_fild');
                if (isset($_POST['lang-slug'])) {
                    $lang_slug = sanitize_text_field($_POST['lang-slug']);
                    $this->deleteTanslate($lang_slug);
                }
                unset($_POST);
            }
            if (isset($_GET['lang-slug'])) {
                $lang_slug = sanitize_text_field($_GET['lang-slug']);
                if (isset($_POST['data']) && is_array($_POST['data'])) {
                    check_admin_referer('save_translation', 'save_translation_fild');
                    $path = $this->SL_langDirPath . $this->SL_langPluginName . '-' . $lang_slug . '.po';
                    $this->replacePoData($path, $_POST['data']);
                }
                if (isset($_POST['synchron']) && $_POST['synchron'] == 'pot_synchron') {
                    check_admin_referer('synchronize_with_pot', 'synchronize_with_pot_fild');                    
                    $_POST['common-locale'] = $lang_slug;
                    $this->createPoFile();
                }
                $data = $this->getPoData($this->SL_langPluginName . '-' . $lang_slug . '.po');
                $lang = $this->getLanguage($lang_slug);
        ...
        public function createPoFile() {
            if (isset($_GET['post_type']) && $_GET['post_type'] == $this->SL_langForPostType) {
                if (isset($_POST['common-locale']) && !empty($_POST['common-locale'])) {
                    if (isset($_POST['synchronize_with_pot_fild'])) {
                        check_admin_referer('synchronize_with_pot', 'synchronize_with_pot_fild');
                    } else if (isset($_POST['add_new_translation_file_fild'])) {
                        check_admin_referer('add_new_translation_file', 'add_new_translation_file_fild');
                    } else {
                        exit;
                    }
                    ...
                    global $current_user;
                    $common_locale = sanitize_text_field($_POST['common-locale']);
                    $lang = $this->getLanguage($common_locale);
                    $c = explode('_', $common_locale);
                    $path = $this->SL_langDirPath . $this->SL_langPluginName . '-' . $common_locale . '.po';
                    if (!file_exists($path) || isset($_POST['synchron'])) {
                        if (file_exists($path) && isset($_POST['synchron']) && $_POST['synchron'] == 'pot_synchron') {
                            $prevPoData = $this->getPoData($this->SL_langPluginName . '-' . $common_locale . '.po');
                            unlink($path);
                        }
                        $potPath = $this->SL_langPluginDirPath . $this->SL_langPluginName . '.pot';
        ...
        private function replacePoData($path, $data) {            
            ...
            if ($handle) {
                while (($line = fgets($handle)) !== false) {
                    $word = sanitize_text_field($data[$i][1]);
...
// Fall back to in-built MO compiler - requires PO is parsed too
        ...
        private function getAllPoFiles($dir) {
            ...
            foreach ($files as $file) {
                $last_path = substr(basename($file), strlen($this->SL_langPluginName) + 1, -3);
        ...
        public static function get_instance($arg1, $arg2) {
            if (null == self::$instance) {
                self::$instance = new self($arg1, $arg2);
            }
